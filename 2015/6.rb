...
def solve(input)
  instructions = input.split("\n").map { |line|
    ...
    ax, ay, bx, by = m[2..].map(&:to_i)
    { instruction: m[1], x: ax..bx, y: ay..by }
  }

  lights = Matrix[*(0...SIZE).map { (0...SIZE).map { false } }]

  instructions.each { |instruction|
    case instruction[:instruction]
    when 'turn on'
      lights[instruction[:x], instruction[:y]] = true
    when 'turn off'
      lights[instruction[:x], instruction[:y]] = false
    when 'toggle'
      lights[instruction[:x], instruction[:y]] = lights.minor(instruction[:x], instruction[:y]).map! { |on| !on }
    end
  }

  p lights.count { |on| on }

  lights = Matrix[*(0...SIZE).map { (0...SIZE).map { 0 } }]

  instructions.each { |instruction|
    case instruction[:instruction]
    when 'turn on'
      lights[instruction[:x], instruction[:y]] = lights.minor(instruction[:x], instruction[:y]).map! { |b| b+1 }
    when 'turn off'
      lights[instruction[:x], instruction[:y]] = lights.minor(instruction[:x], instruction[:y]).map! { |b| [b-1,0].max }
    when 'toggle'
      lights[instruction[:x], instruction[:y]] = lights.minor(instruction[:x], instruction[:y]).map! { |b| b+2 }
    end
  }
